...
def main():
    ...
    device, green_inv = pcv.invert(img=green_channel, device=device, debug=args.debug)
    ...
    device, med_blur = pcv.median_blur(img=blur_thresholded, ksize=5, device=device, debug=args.debug)
    ...
    device, contours, hierarchy = pcv.find_objects(img=img, mask=np.copy(green_fill_50), device=device,
                                                   debug=args.debug)
