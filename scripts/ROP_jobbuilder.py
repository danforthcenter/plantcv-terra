...
def options():
    ...
    parser.add_argument('-c', '--config', help= "json file of background assignments per experiment", required= True)
    ...
    parser.add_argument("-w", "--writeimg", help='Include analysis images in output.', default=False,
                        action="store_true")
    parser.add_argument("-P", "--pdf", help='PDF for Naive-Bayes.', required=True)
    parser.add_argument("-r", "--result_dir", help='Directory for result .txt files', required=True)
    ...
    for batch in range(0, batches):
        ...
        for job in range(job_num, job_num + 1000):
            if job == len(images):
                break
            matrix = ""
            if "TM015" in images[job]:
                background = config["round"]
            elif "TM016" in images[job]:
                background = config["round"]
            else:
                background = config["square"]
            file_parts = images[job].split('/')
            batchfile.write("job_args = --image " + images[job] + " --npz " + background + " --pdf " + args.pdf +
                            + " --outdir "+ os.path.join(args.outdir, file_parts[-2], file_parts[-1]) + "\n")
